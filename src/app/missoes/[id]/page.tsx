...
export default function MissaoDetailPage() {
  ...
  const [requests, setRequests] = useState<MissionRequest[] | null>(null);
  const [myRequest, setMyRequest] = useState<MissionRequest | null>(null);
  ...
  useEffect(() => {
    ...
    apiFetch("/missions/my-requests")
      .then((res) => (res.ok ? res.json() : []))
      .then((all: (MissionRequest & { missionId: string })[]) => {
        setMyRequest(all.find((r) => r.missionId === missionId && r.status === "PENDING") ?? null);
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [missionId]);

  useEffect(() => {
    if (isTopManager && missionId) {
      apiFetch(`/missions/${missionId}/requests`)
        .then((res) => (res.ok ? res.json() : []))
        .then(setRequests);
    }
  }, [isTopManager, missionId]);
  ...
  async function handleUpdate(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await apiFetch(`/missions/${missionId}`, {
      method: "PATCH",
  ...
  async function handleJoin() {
    setJoining(true);
    try {
      const response = await apiFetch(`/missions/${missionId}/requests`, { method: "POST" });
  ...
  async function handleRequestStatus(requestId: string, status: "APPROVED" | "REJECTED") {
    await apiFetch(`/missions/${missionId}/requests/${requestId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  if (notFound) {
    return (
      <main className="px-8 py-8">
        <p className="text-sm text-zinc-500">Missão não encontrada.</p>
        <Link href="/missoes" className="mt-2 inline-block text-sm text-amber-700 hover:underline">
          ← Voltar
        </Link>
      </main>
    );
  ...
  return (
    <main className="px-8 py-8">
      <Link href="/missoes" className="text-sm text-zinc-400 hover:text-zinc-700">
        ← Missões
      </Link>

      <div className="mt-2 flex items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">{mission.name}</h1>
        {isTopManager ? (
          <button
            type="button"
      ...
      {isTopManager ? (
        <form onSubmit={handleUpdate} className="mt-6 flex max-w-lg flex-col gap-4 rounded-2xl border border-zinc-100 p-5">
          <label className="flex flex-col gap-1 text-sm text-zinc-600">
            Nome
            <input name="name" defaultValue={mission.name} required minLength={2} className="rounded-md border border-zinc-200 px-3 py-2 text-zinc-900" />
          </label>
          <RichTextEditor name="description" label="Descrição" defaultValue={mission.description} />
