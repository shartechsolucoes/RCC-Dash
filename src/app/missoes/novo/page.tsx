...
export default function NovaMissaoPage() {
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setErrorMessage("Não foi possível criar a missão");
      return;
    }

    const created = await response.json();
    router.push(`/missoes/${created.id}`);
  }
  ...
  return (
    <main className="px-8 py-8">
      ...
      <h1 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-900">Nova missão</h1>
      ...
      <form onSubmit={handleCreate} className="mt-6 flex max-w-lg flex-col gap-4 rounded-2xl border border-zinc-100 p-5">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/missoes" className="rounded-full border border-zinc-200 px-5 py-2.5 text-sm font-medium text-zinc-600 hover:bg-zinc-50">
            Cancelar
          </Link>
        </div>
      </form>
